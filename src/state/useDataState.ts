...
//     if (!jsonData?.row_nodes) return new Map();
...
//     if (!jsonData?.col_nodes) return new Map();
//     const map = new Map<string, any>();
...
//       console.log('******* json data is as follows ******', jsonData)
//       let transformedData = jsonData;
//       if (jsonData.mat.length > 0) {
//         transformedData = transformClustergrammerData(jsonData,filteredIdxDict);
//       }

//       transformedData = sortData(transformedData, order, jsonData,filteredIdxDict);
...
//       console.log('%%%%%%% in data state col category is as follows %%%%%%%%', colCategory)
//       console.log('%%%%%%% in data state row category is as follows %%%%%%%%', rowCategory)
//       // const colCategory  = ""
//       // const rowCategory = ""
//       let colCatIndex: string[] = [];
//       let rowCatIndex: string[] = [];
//       if (colCategory.length > 0 && colCategory.every((cat:string) => categories["col"].hasOwnProperty(cat))) {
//         for (let i = 0; i < colCategory.length; i++) {
//           colCatIndex.push(categories.col[colCategory[i]]);
//         }
//       }
//       if (rowCategory.length > 0 && rowCategory.every((cat:string) => categories["row"].hasOwnProperty(cat))) {
...
//     }
//     return null;
//   }, [jsonData, order,filteredIdxDict,rowNodeMap, colNodeMap]);
...
export const useDataState = (
  ...
  const dataState = useMemo<DataStateShape | null>(() => {
    ...
    if (jsonData) {
      ...
      // 3) Build a typed array for all numeric values
      const totalSize = numRows * numColumns;
      const typedValues = new Float32Array(totalSize);
  
      let index = 0;
      for (const rowKey of rows) {
        const rowMap = transformedData.get(rowKey); // Map<colKey, number>
        for (const val of rowMap.values()) {
          typedValues[index++] = val;
        }
      }



    // 4) Compute min/max in a single pass
    let min = Infinity;
    let max = -Infinity;
    for (let i = 0; i < typedValues.length; i++) {
      const v = typedValues[i];
      if (v < min) min = v;
      if (v > max) max = v;
    }
      ...
      return {
        ...
        colLabels: columns.map((v, i) => ({
          ...
          // group: ID === 'olinkPatientHeatmap' || ID === 'cytofPatientHeatmap'?jsonData.col_nodes.filter((d: { name: string;})=>d.name.trim() === v.trim())[0]?.["group"]:null,
          group: colNodeMap.get(v.trim())?.['group'],
          visibility:
            Object.keys(categories.row).length > 0 ||
            Object.keys(categories.col).length > 0
              ? false
